...
export default function ProtectedView({
    ...
    return (
        ...
        >
            ...
            {enableWatermark && (
                <div className="no-print no-print-strict pointer-events-none absolute inset-0 z-10 overflow-hidden opacity-[0.06] select-none flex flex-wrap gap-12 p-8 justify-around items-center rotate-[-25deg]">
            ...
            {/* Protected Content */}
            <div className={isBlurred && !isPrinting ? 'filter blur-lg transition-all' : ''} style={{ filter: isPrinting ? 'none !important' : '', opacity: isPrinting ? '1 !important' : '' }}>
                {children}
            </div>
        </div>
    );
