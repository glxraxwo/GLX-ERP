...
export default function SendPaymentSheetSmsModal({ isOpen, onClose, employeeId, phoneNumbers = {}, startDate, endDate }) {
    ...
    const handleShare = async (e) => {
        ...
        setSending(true);
        try {
            const res = await api.post(`/hr/employees/${employeeId}/payment-sheet/send-sms`, {
                phone: finalPhone,
                startDate,
                endDate
            });
            if (res.data && res.data.success) {
                toast.success('Payment sheet summary successfully sent via SMS!');
                onClose();
                // Reset form state
                setUseCustom(false);
                setCustomPhone('');
            } else {
                toast.error('Failed to send payment sheet summary.');
            }
        } catch (err) {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Send Payment Sheet Summary via SMS" size="sm">
            <form onSubmit={handleShare} className="space-y-4 font-calibri">
                <p className="text-xs text-gray-500">
                    This will send an instant SMS containing the payment sheet summary (total salary, advances, net salary) for the selected period. A copy will also be dispatched to the Manager.
                </p>
                ...
                <div className="flex justify-end gap-2 pt-2">
                    ...
                    <Button type="submit" variant="primary" size="sm" loading={sending}>
                        <Send size={14} className="mr-1" /> Send Summary
                    </Button>
                </div>
            </form>
        </Modal>
