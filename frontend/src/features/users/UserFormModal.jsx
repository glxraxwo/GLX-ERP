...
import { useCreateUser, useUpdateUser, useUploadUserDocument, useDeleteUserDocument } from './useUsers';
...
export default function UserFormModal({ isOpen, onClose, user = null }) {
    ...
    useEffect(() => {
        if (isOpen && user) {
            ...
            setDocumentFile(user.document || null);
    ...
    // Auto-select NIC document type when designation is selected for new users
    useEffect(() => {
        if (!isEdit && designationValue && !documentType) {
            setDocumentType('nic');
        }
    }, [designationValue, documentType, isEdit]);

    const onSubmit = async (data) => {
        // Validate that NIC document is uploaded when creating an employee (with designation)
        // Skip validation for admin users
        if (!isEdit && data.designationId && data.role !== 'admin') {
            if (!documentType || documentType !== 'nic') {
                setDocumentError('NIC document type must be selected');
                return;
            }
            if (!pendingDocument && !documentFile) {
                setDocumentError('NIC document upload is required for employee profiles');
                return;
            }
        }
        ...
        try {
            ...
            // Upload document after user creation if there's a pending document
            if (!isEdit && result?.data?._id && pendingDocument && documentType) {
                try {
                    ...
                    console.error('Document upload failed after user creation:', uploadError);
                }
            }
    ...
    const handleDeleteDocument = async () => {
        if (isEdit && user?._id) {
            try {
                await deleteDocumentMutation.mutateAsync(user._id);
            } catch (error) {
                ...
                <div className="p-6 space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                            Document (ලේඛන)
                            {!isEdit && designationValue && <span className="text-red-500 ml-1">*</span>}
                        </label>
                        <div className="space-y-3">
                            <Select
                                label="Document Type (ලේඛන වරය)"
                            ...
                            />
                            {!isEdit && designationValue && (
                                <p className="text-xs text-amber-700 bg-amber-50 p-2 rounded border border-amber-200">
                                    NIC document is required when creating an employee profile
                                </p>
                            )}
                            {documentFile ? (
                                <div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm text-green-800">Document selected: {documentFile}</span>
                                    </div>
                                    <button
                                        type="button"
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        disabled={uploadDocumentMutation.isPending || !documentType}
                                    />
                                    <label
                                        ...
                                        className={`flex items-center justify-center gap-2 p-4 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${
                                            !documentType || uploadDocumentMutation.isPending ? 'bg-gray-100 cursor-not-allowed opacity-50' : 'hover:border-primary-400 hover:bg-primary-50'
                                        }`}
                                    >
                                        <Upload size={16} className={documentType ? 'text-primary-600' : 'text-gray-400'} />
                                        <span className="text-sm text-gray-600">
                                            {!documentType ? 'Select document type first' : uploadDocumentMutation.isPending ? 'Uploading...' : 'Click to upload document from desktop'}
                                        </span>
                                    </label>
                                </div>
                            )}
                            {documentError && (
                                <p className="text-xs text-red-600 bg-red-50 p-2 rounded border border-red-200">{documentError}</p>
                            )}
                            <p className="text-xs text-gray-500 mt-1">Upload one document per user (PDF, JPG, PNG, DOC, DOCX - Max 5MB)</p>
                        </div>
                    </div>
