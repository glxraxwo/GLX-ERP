...
const usersApi = {
    ...
    uploadDocument: async (id, file, documentType) => {
        ...
        return (await api.post(`/users/${id}/document`, formData, {
            headers: { 'Content-Type': 'multipart/form-data' }
        })).data;
    },
    deleteDocument: async (id) => (await api.delete(`/users/${id}/document`)).data,
};
...
export const useUploadUserDocument = () => {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ id, file, documentType }) => usersApi.uploadDocument(id, file, documentType),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['users'] });
...
export const useDeleteUserDocument = () => {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (id) => usersApi.deleteDocument(id),
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['users'] });
            toast.success('Document deleted');
        },
        onError: (err) => toast.error(err.response?.data?.message || 'Failed to delete document'),
    });
};