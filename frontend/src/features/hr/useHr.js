...
export const useOngoingSalaryPeriod = (params = {}) => useQuery({
    queryKey: ['ongoingSalaryPeriod', params],
    queryFn: () => advancesApi.getOngoingPeriod(params),
    retry: 1,
    onError: (err) => {
        console.error('Failed to load ongoing salary period:', err);
    }
});
...
export const useAdvanceActions = () => {
    ...
    return {
        ...
        pay: useMutation({
            mutationFn: ({ id, paymentData }) => advancesApi.pay(id, paymentData),
            onSuccess: () => { invalidate(qc, ['salaryAdvances', 'payments'])(); toast.success('Advance Payment Processed'); },
            onError: onErr,
        }),
    };
};
...
// Employee Documents
export const useEmployeeDocuments = (employeeId) => useQuery({
...
export const useUploadEmployeeDocument = () => {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ employeeId, file, documentType, title }) => {
        ...
        onSuccess: () => {
            invalidate(qc, ['employeeDocuments', 'employee'])();
...
export const useDeleteEmployeeDocument = () => {
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ employeeId, documentType, documentId }) => 
            employeesApi.deleteDocument(employeeId, { documentType, documentId }),
        onSuccess: () => {
            invalidate(qc, ['employeeDocuments', 'employee'])();
