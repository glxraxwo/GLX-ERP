...
export default function ExpensesPage() {
  ...
  const fetchLookups = async () => {
    try {
      const [prodRes, whRes, bankRes] = await Promise.all([
      ...
      console.error('Failed to load products/warehouses/bank accounts in expenses page', err);
    }
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {formData.paymentMethod === 'Bank Transfer' && (
                <div className="p-3 bg-indigo-50 rounded-lg space-y-3">
                  <div>
                    <label className="block text-xs font-semibold text-indigo-800 mb-1">Bank Account *</label>
                    <select
                      required
                      ...
                      <div className="bg-white p-3 rounded-lg border border-indigo-200 space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          <div>
                            ...
                            <p className="font-semibold text-slate-800">{selectedAccount.bankName}</p>
                          </div>
                          <div>
                            <span className="text-gray-500">Account Number:</span>
                            <p className="font-semibold text-slate-800">{selectedAccount.accountNumber}</p>
                          </div>
                          <div>
