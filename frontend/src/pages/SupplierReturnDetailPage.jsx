...
export default function SupplierReturnDetailPage() {
    ...
    const handleCredit = async () => {
        ...
        if (sr.supplierId?._id) {
            queryClient.invalidateQueries({ queryKey: ['supplier', sr.supplierId._id] });
        }
    };
                ...
                <div>
                    ...
                    {sr.supplierId && (
                        <Card className="p-6 mt-6">
                            <h3 className="text-sm font-semibold mb-4">Supplier Balance Impact</h3>
                            <div className="space-y-2 text-sm">
                                <div className="flex justify-between"><span className="text-gray-600">Supplier Balance Due</span>
                                    <span className="font-semibold">{fmt(sr.supplierId.balanceDueLKR || 0)}</span>
                                </div>
                                {sr.status === 'sent' && (
                                    <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg mt-3">
                                        <p className="text-xs text-blue-800 mb-1">Expected Balance After Credit:</p>
                                        <p className="text-sm font-bold text-blue-900">
                                            {fmt(Math.max(0, (sr.supplierId.balanceDueLKR || 0) - sr.expectedCreditAmount))}
                                        </p>
                                    </div>
                                )}
                                {sr.status === 'credit_received' && sr.actualCreditReceived > 0 && (
                                    <div className="p-3 bg-green-50 border border-green-200 rounded-lg mt-3">
                                        <p className="text-xs text-green-800 mb-1">Balance After Credit Received:</p>
                                        <p className="text-sm font-bold text-green-900">
                                            {fmt(Math.max(0, (sr.supplierId.balanceDueLKR || 0) - sr.actualCreditReceived))}
                                        </p>
                                    </div>
                                )}
                            </div>
