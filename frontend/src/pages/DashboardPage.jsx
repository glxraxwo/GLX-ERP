...
function EmployeeDashboard() {
    ...
    const submitAdvance = async () => {
        ...
        if (advanceForm.advanceType === 'percentage') {
            ...
            calcAmount = ((baseSalary * Number(advanceForm.requestedPercentage)) / 100).toFixed(2);
            finalAmount = Number(calcAmount);
        } else {
            finalAmount = Number(advanceForm.amount);
        }
    ...
    const payslips = payslipsRes?.data?.data || payslipsRes?.data || [];
    ...
    const ledger = Array.isArray(ledgerData?.ledger) ? ledgerData.ledger : (Array.isArray(ledgerData) ? ledgerData : []);
