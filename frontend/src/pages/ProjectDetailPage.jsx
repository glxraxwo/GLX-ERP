...
export default function ProjectDetailPage() {
    ...
    const fetchProjectDetails = async () => {
        ...
        try {
            ...
            // Fetch all products for material selection
            try {
                const productsRes = await api.get('/products?status=all');
    ...
    const handleAddMaterial = async (e) => {
        e.preventDefault();
        if (!materialProductId || materialQty <= 0) {
        ...
        try {
            const payload = {
                ...
                productCode: materialProduct?.productCode || '',
                productName: materialProduct?.name || '',
                qty: Number(materialQty),
                buyingPrice: Number(materialBuyingPrice) || 0
            };
            await api.post(`/projects/${id}/issue-material`, payload);
            toast.success('Material issued successfully');
            setIsMaterialOpen(false);
            // Reset form
            setMaterialProductId('');
            setMaterialProduct(null);
            setMaterialQty(1);
            setMaterialBuyingPrice(0);
            fetchProjectDetails();
        } catch (err) {
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                {project.status !== 'delivered' && (
                                    <Button variant="primary" size="sm" onClick={() => setIsMaterialOpen(true)}>
                                        <Plus size={14} className="mr-1" /> Issue Material
                                    </Button>
                                )}
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleAddMaterial} className="space-y-4">
                            <ProductAutocompleteSelect
                                ...
                                onChange={(id, product) => {
                                    ...
                                    setMaterialProduct(product);
                                    if (product) {
                                        setMaterialBuyingPrice(product.costs?.lastPurchaseCost || product.costs?.averageCost || 0);
                                    }
                                }}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <Input label="Quantity" type="number" required value={materialQty} onChange={(e) => setMaterialQty(e.target.value)} min="1" step="1" />
