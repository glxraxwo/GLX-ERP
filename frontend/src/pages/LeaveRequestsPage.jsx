...
import { Plus, CheckCircle, XCircle, Ban, Plane, DollarSign, AlertTriangle, Percent, UserCheck, CreditCard, MinusCircle } from 'lucide-react';
...
export default function LeaveRequestsPage() {
    ...
    const [paymentForm, setPaymentForm] = useState({ method: 'cash', bankAccountId: '', notes: '' });
    const [actionReason, setActionReason] = useState('');
    const [unauthorizedModal, setUnauthorizedModal] = useState(null);
    const [unauthorizedForm, setUnauthorizedForm] = useState({ deductionAmount: 0, notes: '' });
    ...
    const bankAccountOptions = bankAccounts.map(acc => ({
        value: acc._id,
        label: `${acc.bankName} - ${acc.accountNumber} (${acc.accountType})`
    }));
    ...
    const handlePayment = async () => {
        const { advance } = paymentModal;
        try {
            await advanceActions.pay.mutateAsync({ 
                id: advance._id, 
                paymentData: {
                    method: paymentForm.method,
                    bankAccountId: paymentForm.bankAccountId || undefined,
                    notes: paymentForm.notes || `Salary advance payment for ${advance.employeeId?.firstName} ${advance.employeeId?.lastName}`
                }
            });
            setPaymentModal(null);
            setPaymentForm({ method: 'cash', bankAccountId: '', notes: '' });
        } catch (err) {
            console.error('Payment error:', err);
        }
    };

    const handleUnauthorizedDeduction = async () => {
        const { leave } = unauthorizedModal;
        try {
            await api.put(`/hr/leave-requests/${leave._id}/unauthorized-deduction`, {
                isUnauthorized: true,
                unauthorizedDeductionAmount: unauthorizedForm.deductionAmount,
                unauthorizedDeductionNotes: unauthorizedForm.notes,
            });
            toast.success('Unauthorized leave deduction updated successfully');
            setUnauthorizedModal(null);
            setUnauthorizedForm({ deductionAmount: 0, notes: '' });
        } catch (err) {
            console.error('Unauthorized deduction error:', err);
            toast.error('Failed to update unauthorized leave deduction');
        }
    };
    ...
    const leaveColumns = [
        ...
        },
        {
            key: 'type', label: 'Type', render: (r) => (
                <div className="flex items-center gap-1.5">
                    <Badge variant={r.isUnauthorized ? 'danger' : (r.isUninformed ? 'warning' : 'info')}>
                        {r.isUnauthorized ? '🚫 Unauthorized' : (r.isUninformed ? '⚠️ Uninformed' : r.leaveType)}
                    </Badge>
                </div>
            )
        },
        ...
        {
            key: 'actions', label: 'Actions', width: '150px', render: (r) => (
                <div className="flex gap-1">
                    ...
                    {['approved'].includes(r.status) && canApprove && (
                        <button onClick={() => setUnauthorizedModal({ leave: r })}
                            className="p-1.5 hover:bg-red-50 text-red-600 rounded" title="Mark as Unauthorized / Set Deduction"><MinusCircle size={16} /></button>
                    )}
    ...
    const advanceColumns = [
        ...
        {
            key: 'deduction', label: 'Payment Status', render: (r) => (
                <Badge variant={r.isDeducted ? 'success' : 'warning'}>
                    {r.isDeducted ? 'Paid & Deducted' : 'Pending Payment'}
                </Badge>
            )
        },
        {
            key: 'actions', label: 'Actions', width: '150px', render: (r) => (
                <div className="flex gap-1">
                    ...
                    {r.status === 'approved' && !r.isDeducted && canApprove && (
                        <button onClick={() => setPaymentModal({ advance: r })}
                            className="p-1.5 hover:bg-blue-50 text-blue-600 rounded" title="Pay Advance"><CreditCard size={16} /></button>
                    )}
                </div>
            )
        },
    ...
    return (
        <div>
            ...
            {/* Payment Modal for Approved Advances */}
            <Modal isOpen={!!paymentModal} onClose={() => { setPaymentModal(null); setPaymentForm({ method: 'cash', bankAccountId: '', notes: '' }); }} title="Process Advance Payment" size="md">
                <div className="p-6 space-y-4">
                    <div className="bg-blue-50 border border-blue-200 rounded p-4">
                        <p className="text-sm text-gray-700 mb-2">
                            <strong>Employee:</strong> {paymentModal?.advance?.employeeId?.firstName} {paymentModal?.advance?.employeeId?.lastName}
                        </p>
                        <p className="text-sm text-gray-700 mb-2">
                            <strong>Amount:</strong> {fmtMoney(paymentModal?.advance?.amount)}
                        </p>
                        <p className="text-sm text-gray-700 mb-2">
                            <strong>Request Type:</strong> {paymentModal?.advance?.advanceType === 'percentage' ? `${paymentModal?.advance?.requestedPercentage}% Share` : 'Fixed Amount'}
                        </p>
                        <p className="text-sm text-gray-700">
                            <strong>Reason:</strong> {paymentModal?.advance?.reason || 'No reason provided'}
                        </p>
                    </div>
                    ...
                    <Select label="Payment Method"
                        ...
                        onChange={(e) => setPaymentForm((f) => ({ ...f, method: e.target.value, bankAccountId: '' }))} />
                    
                    {(paymentForm.method === 'bank_transfer' || paymentForm.method === 'cheque') && (
                        <Select 
                    ...
                    <Textarea label="Payment Notes" rows={2} placeholder="Optional notes..." value={paymentForm.notes}
                ...
                <div className="flex justify-end gap-2 px-6 py-4 border-t bg-gray-50">
                    ...
                    <Button 
                        ...
                        loading={advanceActions.pay.isPending}
                        disabled={(paymentForm.method === 'bank_transfer' || paymentForm.method === 'cheque') && !paymentForm.bankAccountId}
                    >
                        <CreditCard size={16} className="mr-1.5" /> Process Payment
                    </Button>
                </div>
            </Modal>

            {/* Unauthorized Leave Deduction Modal */}
            <Modal isOpen={!!unauthorizedModal} onClose={() => { setUnauthorizedModal(null); setUnauthorizedForm({ deductionAmount: 0, notes: '' }); }} title="Unauthorized Leave Deduction (අවසරයකින් තොරව ගත් නිවාඩු සඳහා වැටුපෙන් කපන මුදල)" size="md">
                <div className="p-6 space-y-4">
                    <div className="bg-red-50 border border-red-200 rounded p-4">
                        <p className="text-sm text-gray-700 mb-2">
                            <strong>Employee:</strong> {unauthorizedModal?.leave?.employeeName}
                        </p>
                        <p className="text-sm text-gray-700 mb-2">
                            <strong>Leave Type:</strong> {unauthorizedModal?.leave?.leaveType}
                        </p>
                        <p className="text-sm text-gray-700 mb-2">
                            <strong>Dates:</strong> {new Date(unauthorizedModal?.leave?.fromDate).toLocaleDateString('en-LK')} — {new Date(unauthorizedModal?.leave?.toDate).toLocaleDateString('en-LK')}
                        </p>
                        <p className="text-sm text-gray-700 mb-2">
                            <strong>Number of Days:</strong> {unauthorizedModal?.leave?.numberOfDays}
                        </p>
                        <p className="text-sm text-gray-700">
                            <strong>Current Deduction:</strong> {fmtMoney(unauthorizedModal?.leave?.unauthorizedDeductionAmount || 0)}
                        </p>
                    </div>
                    
                    <Input 
                        label="Deduction Amount (LKR)" 
                        type="number" 
                        step="100" 
                        min="0" 
                        value={unauthorizedForm.deductionAmount}
                        onChange={(e) => setUnauthorizedForm((f) => ({ ...f, deductionAmount: Number(e.target.value) }))} 
                    />
                    
                    <Textarea 
                        label="Deduction Notes / Reason" 
                        rows={2} 
                        placeholder="Reason for this deduction..." 
                        value={unauthorizedForm.notes}
                        onChange={(e) => setUnauthorizedForm((f) => ({ ...f, notes: e.target.value }))} 
                    />
