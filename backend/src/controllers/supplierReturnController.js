...
export const getSupplierReturnById = asyncHandler(async (req, res) => {
    const sr = await SupplierReturn.findById(req.params.id)
        .populate('supplierId', 'displayName supplierCode balanceDueLKR')
        .populate('warehouseId', 'name warehouseCode')
        .populate('items.productId', 'name productCode')
...
export const recordSupplierCredit = asyncHandler(async (req, res) => {
    ...
    // Update supplier balance when credit is actually received
    if (actualCreditReceived > 0) {
        const supplier = await Supplier.findById(sr.supplierId);
        if (supplier) {
            supplier.balanceDueLKR = +(Math.max(0, (supplier.balanceDueLKR || 0) - actualCreditReceived)).toFixed(2);
            await supplier.save();
        }
    }
