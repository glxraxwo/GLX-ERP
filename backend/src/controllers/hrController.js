...
import { sendPaymentSheetSms } from '../services/smsService.js';
...
export const getEmployees = asyncHandler(async (req, res) => {
    ...
    // Auto-sync any existing active User accounts that do not have an Employee record
    try {
        const users = await User.find({ deletedAt: null, isActive: true });
        const existingEmpUserIds = (await Employee.find().select('userId')).map(e => e.userId?.toString()).filter(Boolean);
        for (const u of users) {
            if (!existingEmpUserIds.includes(u._id.toString())) {
                await Employee.create({
                    ...
                    epfRate: 8,
                    etfRate: 3,
                    createdBy: u.createdBy || u._id,
                    dateOfJoining: u.createdAt || new Date(),
                }).catch(() => {});
            }
        }
    } catch (syncErr) {
        console.error('Error auto-syncing users to employees:', syncErr);
    }
        ...
        for (let date = new Date(fromDate); date <= toDate; date.setDate(date.getDate() + 1)) {
            ...
            if (existingAttendance) {
                ...
                existingAttendance.notes = existingAttendance.notes 
                    ? `${existingAttendance.notes} | Leave: ${leave.reason || leave.leaveType}`
                    : `Leave: ${leave.reason || leave.leaveType}`;
                ...
                const leaveAttendance = new Attendance({
                    ...
                    leaveId: leave._id,
                    notes: `Leave: ${leave.reason || leave.leaveType}`,
                    checkInTime: null,
                    checkOutTime: null,
...
export const cancelLeaveRequest = asyncHandler(async (req, res) => {
    ...
    if (wasApproved) {
        ...
        if (emp) {
            ...
            // Remove or update attendance records for the cancelled leave
            const fromDate = new Date(leave.fromDate);
            const toDate = new Date(leave.toDate);
            ...
            for (let date = new Date(fromDate); date <= toDate; date.setDate(date.getDate() + 1)) {
                ...
                if (attendance) {
                    // If the attendance was specifically created for this leave, delete it
                    if (attendance.status === 'leave' && attendance.leaveId && attendance.leaveId.toString() === leave._id.toString()) {
                        await Attendance.findByIdAndDelete(attendance._id);
                    } else {
                        // If attendance existed before and was updated, revert the leave-related changes
                        attendance.status = 'present';
                        attendance.leaveType = undefined;
                        attendance.leaveId = undefined;
                        attendance.notes = attendance.notes?.replace(/ \| Leave:.*$/g, '')?.replace(/^Leave:.*$/g, '') || '';
...
export const updateUnauthorizedLeaveDeduction = asyncHandler(async (req, res) => {
    const { isUnauthorized, unauthorizedDeductionAmount, unauthorizedDeductionNotes } = req.body;
    const leave = await LeaveRequest.findById(req.params.id);
    if (!leave) { res.status(404); throw new Error('Leave not found'); }
    if (leave.status !== 'approved') {
        res.status(400); throw new Error('Can only set deduction for approved leaves');
    }

    leave.isUnauthorized = isUnauthorized;
    leave.unauthorizedDeductionAmount = unauthorizedDeductionAmount || 0;
    leave.unauthorizedDeductionNotes = unauthorizedDeductionNotes;
    await leave.save();
...
export const paySalaryAdvance = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { method, bankAccountId, notes, transactionReference } = req.body;
    ...
    if (advance.isDeducted) {
        res.status(400);
        throw new Error('This advance has already been paid and deducted');
    }

    const emp = await Employee.findById(advance.employeeId);
    if (!emp) {
        res.status(404);
        throw new Error('Employee not found');
    }

    // Import Payment and BankAccount models
    const Payment = (await import('../models/Payment.js')).default;
    const BankAccount = (await import('../models/BankAccount.js')).default;

    // Clean up bankAccountId - convert empty string to undefined
    const cleanBankAccountId = bankAccountId && bankAccountId.trim() !== '' ? bankAccountId : undefined;

    // Update bank account balance if provided
    if (cleanBankAccountId) {
        const bankAccount = await BankAccount.findById(cleanBankAccountId);
        if (!bankAccount) throw new Error('Bank account not found');

        bankAccount.balance = +(bankAccount.balance - Number(advance.amount)).toFixed(2);
    ...
    const payment = await Payment.create({
        ...
        voucherCategory: 'salary_advance',
        employeeId: advance.employeeId,
        partyName: `${emp.firstName || ''} ${emp.lastName || ''}`.trim() || emp.employeeCode || 'Employee',
        bankAccountId: cleanBankAccountId,
        amount: advance.amount,
        method: method || 'cash',
        notes: notes || `Salary advance payment for ${emp.firstName} ${emp.lastName}`,
    ...
    res.json({ success: true, data: { advance, payment: populatedPayment } });
});
...
/**
 * Get ongoing salary period information for advance request
 * This provides the current salary period details including:
 * - Current ongoing salary period (month/year)
 * - Maximum advance amount for that period
 * - Already taken advances for that period
 * - Remaining available advance amount
 */
export const getOngoingSalaryPeriodInfo = asyncHandler(async (req, res) => {
    let targetEmployeeId = req.query.employeeId || req.params.employeeId;
    ...
    // Import Payroll model to check existing payroll periods
    const Payroll = (await import('../models/Payroll.js')).default;
    const Settings = (await import('../models/Settings.js')).default;

    // Get current date and determine ongoing salary period
    const now = new Date();
    const currentMonth = now.getMonth() + 1; // 1-12
    const currentYear = now.getFullYear();

    // Get settings to determine payment day
    const settings = await Settings.findOne();
    const paymentDay = settings?.salaryPaymentDay || 25;

    // Determine ongoing salary period
    // If current day is before payment day, ongoing period is current month
    // If current day is after payment day, ongoing period is next month
    let ongoingMonth, ongoingYear;
    
    if (now.getDate() < paymentDay) {
        ongoingMonth = currentMonth;
        ongoingYear = currentYear;
    } else {
        if (currentMonth === 12) {
            ongoingMonth = 1;
            ongoingYear = currentYear + 1;
        } else {
            ongoingMonth = currentMonth + 1;
            ongoingYear = currentYear;
        }
    }

    // Calculate period dates
    const periodStartDate = new Date(ongoingYear, ongoingMonth - 1, 1);
    const periodEndDate = new Date(ongoingYear, ongoingMonth, 0, 23, 59, 59, 999);

    // Get employee's basic salary for the period
    // For daily/hourly workers, calculate based on their rate
    let ongoingSalary = emp.basicSalary || 0;
    
    // If employee has labourRate (daily/hourly worker), calculate monthly equivalent
    if (emp.labourRate && emp.labourRate > 0) {
        if (emp.paymentType === 'per_day') {
            ongoingSalary = emp.labourRate * 26; // Assume 26 working days per month
        } else if (emp.paymentType === 'per_hour') {
            ongoingSalary = emp.labourRate * 200; // Assume 200 working hours per month
        }
    }
    ...
    const existingAdvances = await SalaryAdvance.find({
        employeeId: targetEmployeeId,
        status: { $in: ['approved', 'pending'] },
        date: { $gte: periodStartDate, $lte: periodEndDate }
    });

    const alreadyTakenAdvance = existingAdvances.reduce((sum, adv) => sum + (adv.amount || 0), 0);

    // Calculate remaining available advance
    const remainingAdvanceAmount = Math.max(0, maxAdvanceAmount - alreadyTakenAdvance);

    // Get month name for display
    const monthNames = ['January', 'February', 'March', 'April', 'May', 'June',
        'July', 'August', 'September', 'October', 'November', 'December'];
    const monthName = monthNames[ongoingMonth - 1];
    ...
    res.json({
        ...
        data: {
            employee: {
                ...
                basicSalary: ongoingSalary,
                paymentType: emp.paymentType || 'monthly',
                labourRate: emp.labourRate || 0,
            },
            salaryPeriod: {
                month: ongoingMonth,
                year: ongoingYear,
                ...
                displayPeriod: `${monthName} ${ongoingYear}`
            },
            advanceLimits: {
                ongoingSalary: ongoingSalary,
                maxAdvancePercentage: maxAdvancePercentage,
                maxAdvanceAmount: maxAdvanceAmount,
                alreadyTakenAdvance: alreadyTakenAdvance,
                remainingAdvanceAmount: remainingAdvanceAmount,
                advancesCount: existingAdvances.length
            }
        }
    });
});
...
/**
 * @desc    Send payment sheet summary via SMS to employee
 * @route   POST /api/hr/employees/:id/payment-sheet/send-sms
 * @access  Private (hr.employees.view)
 */
export const sendPaymentSheetLinkSms = asyncHandler(async (req, res) => {
    const { id } = req.params;
    ...
    attendanceLogs.forEach(log => {
        ...
        if (dateMap[dateStr]) {
            ...
            if (log.checkInTime && log.checkOutTime) {
                ...
                inStr = inDate.toTimeString().split(' ')[0];
                outStr = outDate.toTimeString().split(' ')[0];
    ...
    // Calculate totals
    const paymentRows = Object.values(dateMap);
    const totalSalary = paymentRows.reduce((sum, r) => sum + r.daySalary, 0);
    const totalAdvances = paymentRows.reduce((sum, r) => sum + r.advance, 0);
    const netSalary = totalSalary - totalAdvances;

    // Get host origin from request headers
    const hostOrigin = `${req.protocol}://${req.get('host')}`;

    // Send SMS
    const result = await sendPaymentSheetSms(
        {
    ...
    res.json({
        ...
        message: result.success ? 'Payment sheet summary sent via SMS successfully' : 'Failed to send payment sheet SMS',
        status: result.status
    });
});

// ============================================================
// PUBLIC PAYSLIP ACCESS
// ============================================================

export const getPublicPayslip = asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { startDate, endDate } = req.query;
...
export const uploadEmployeeDocument = asyncHandler(async (req, res) => {
    ...
    const fileUrl = `/uploads/employee-documents/${documentType === 'gsCertificate' ? 'gs-certificates' : 
                      documentType === 'educationCertificates' ? 'education-certificates' : 
                      documentType === 'policeReport' ? 'police-reports' : 'general'}/${req.file.filename}`;

    // Update the specific document field based on type
    if (documentType === 'gsCertificate') {
        employee.gsCertificate = {
            ...employee.gsCertificate,
            status: 'submitted',
            url: fileUrl,
            uploadedAt: new Date()
        ...
        employee.educationCertificates = {
            ...employee.educationCertificates,
            status: 'submitted',
            url: fileUrl,
            uploadedAt: new Date()
        ...
        // Add to general documents array
...
export const getEmployeeDocuments = asyncHandler(async (req, res) => {
    ...
    const documents = {
        gsCertificate: employee.gsCertificate,
        educationCertificates: employee.educationCertificates,
        policeReport: employee.policeReport,
        generalDocuments: employee.documents || []
    };
...
export const deleteEmployeeDocument = asyncHandler(async (req, res) => {
    ...
    if (documentType === 'gsCertificate') {
        employee.gsCertificate.url = '';
        employee.gsCertificate.status = 'pending';
    } else if (documentType === 'educationCertificates') {
        employee.educationCertificates.url = '';
        employee.educationCertificates.status = 'pending';
    } else if (documentType === 'policeReport') {
        employee.policeReport.url = '';
        employee.policeReport.status = 'pending';
    } else if (documentId) {
