...
import { sendPayslipSms } from '../services/smsService.js';

/**
 * Helper: calculate scheduled salary payment date based on settings
 */
const calculateScheduledPaymentDate = async (year, month) => {
    const settings = await Settings.findOne();
    if (!settings) {
        // Default to 25th if no settings exist
        return new Date(year, month, 25);
    }

    if (settings.salaryPaymentDayType === 'last_working_day') {
        // Find last working day of the month (excluding Sundays and holidays)
        const lastDay = new Date(year, month, 0); // Last day of the month
        const holidays = await Holiday.find({
            date: { $gte: new Date(year, month - 1, 1), $lte: lastDay },
        ...
        for (let d = new Date(lastDay); d.getMonth() === month - 1; d.setDate(d.getDate() - 1)) {
        ...
        // Fallback to last day if no working day found
        return lastDay;
    } else {
        // Fixed day payment
        const paymentDay = Math.min(settings.salaryPaymentDay || 25, new Date(year, month, 0).getDate());
        return new Date(year, month, paymentDay);
    }
};

/**
 * Helper: determine next salary period to process based on current date and settings
 */
const getNextSalaryPeriodHelper = async () => {
    const settings = await Settings.findOne();
    ...
    if (currentDay >= paymentDay) {
        // We're past the payment day, so process next month
        if (currentMonth === 12) {
            targetMonth = 1;
        ...
        // We're before the payment day, so process current month
    ...
    if (existingPayroll) {
        ...
        // Check if the next month also exists
        const nextExisting = await Payroll.findOne({
            periodMonth: targetMonth,
            periodYear: targetYear,
            deletedAt: null
        });
        ...
        if (nextExisting) {
            // If both current and next month exist, return null (no available period)
            return null;
        }
    }

    const scheduledPaymentDate = await calculateScheduledPaymentDate(targetYear, targetMonth);
...
const getEmployeeMonthAttendance = async (employeeId, year, month) => {
    ...
    approvedLeaves.forEach((l) => {
        ...
        if (l.isUnauthorized) unauthorizedLeaveDays += actualDays;
    });
...
export const processPayroll = asyncHandler(async (req, res) => {
    ...
    for (const emp of employees) {
        ...
        // Get unauthorized leave deductions
        const unauthorizedLeaves = await LeaveRequest.find({
            employeeId: emp._id,
            status: 'approved',
            isUnauthorized: true,
        ...
        let totalUnauthorizedDeduction = 0;
        unauthorizedLeaves.forEach((ul) => {
            const deductionAmt = ul.unauthorizedDeductionAmount || 0;
            if (deductionAmt > 0) {
                totalUnauthorizedDeduction += deductionAmt;
                otherDeductions.push({
                    name: `Unauthorized Leave Deduction (${ul.leaveType} - ${ul.numberOfDays} day${ul.numberOfDays > 1 ? 's' : ''})`,
                    amount: deductionAmt,
        ...
        payslips.push({
            ...
            uninformedLeaveDays: attendance.unauthorizedLeaveDays || 0,
            overtimeHours: attendance.overtimeHours,
            ...
            unauthorizedLeaveDeducted: totalUnauthorizedDeduction,
            netPay: calc.netPay,
    ...
    const scheduledPaymentDate = await calculateScheduledPaymentDate(periodYear, periodMonth);

    const payroll = new Payroll({
        periodMonth, periodYear, periodStartDate, periodEndDate, scheduledPaymentDate,
        payslips,
        status: 'processed',
    ...
    // Calculate and return the next salary period
    const nextPeriod = await getNextSalaryPeriodHelper();

    res.status(201).json({ success: true, data: payroll, nextPeriod });
});
...
export const markPayrollPaid = asyncHandler(async (req, res) => {
    ...
    // Check if payment is before scheduled date
    const settings = await Settings.findOne();
    const allowEarlyPayment = settings?.allowEarlySalaryPayment || false;
    const scheduledDate = p.scheduledPaymentDate;
    const currentDate = new Date();

    if (!allowEarlyPayment && scheduledDate && currentDate < scheduledDate) {
        res.status(400);
        throw new Error(`Salary cannot be paid before the scheduled payment date of ${scheduledDate.toLocaleDateString('en-LK')}. Please enable early payment in settings or wait until the scheduled date.`);
    }
    ...
    res.json({ success: true, data: p, nextPeriod });
});
...
/**
 * GET /api/payroll/next-period
 * Get the next salary period that should be processed
 */
export const getNextSalaryPeriodInfo = asyncHandler(async (req, res) => {
    const nextPeriod = await getNextSalaryPeriodHelper();
    
    if (!nextPeriod) {
        return res.json({ success: true, data: null, message: 'No available salary period to process' });
    }

    res.json({ success: true, data: nextPeriod });
});
...
export const getPublicPayslipByToken = asyncHandler(async (req, res) => {
    ...
    ).populate('payslips.employeeId', 'firstName lastName employeeCode departmentId designationId phone secondaryPhone tertiaryPhone email bankDetails gsCertificate policeReport educationCertificates paymentType labourRate');
...
/**
 * @desc    Send payslip link via SMS to employee
 * @route   POST /api/payroll/payslips/:payslipId/send-sms
 * @access  Private (hr.payroll.view)
 */
export const sendPayslipLinkSms = asyncHandler(async (req, res) => {
    ...
    // Find the payroll containing this payslip
    const payroll = await Payroll.findOne(
        { 'payslips._id': payslipId, deletedAt: null },
    ...
    // Send SMS
    const result = await sendPayslipSms(
        {
            payslip,
            payrollNumber: payroll.payrollNumber,
            periodMonth: payroll.periodMonth,
    ...
    res.json({
        ...
        message: result.success ? 'Payslip link sent via SMS successfully' : 'Failed to send payslip link SMS',
